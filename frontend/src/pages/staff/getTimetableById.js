...
export const GetTimetableById = () => {
    ...
    useEffect(() => {
        const fetchTimetableData = async () => {
            try {
                const response = await fetch(`http://localhost:3001/staff/${userId}/timetable/${timetableID}`, {
                    method: 'GET',
                    headers: {
    ...
    const handleDelete = async () => {
        try {
            const response = await fetch(`http://localhost:3001/staff/${userId}/timetable/delete/${timetableID}`, {
                method: 'PATCH',
                headers: {
                    Authorization: `Bearer ${token}`,
                    'Content-Type': 'application/json',
