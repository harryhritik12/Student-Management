...
export const CourseById = () => {
    ...
    const handleOnClickDelete = async (courseId) => {
        ...
        try {
            const response = await fetch(`http://localhost:3001/faculty/${userId}/additionalCourse/delete/${courseId}`, {
                method: 'PATCH',
                headers: {
    ...
    useEffect(() => {
        const fetchCourse = async () => {
            try {
                const response = await fetch(`http://localhost:3001/faculty/${courseId}/additionalCourse/${courseId}`, {
                    method: 'GET',
                    headers: {
                        Authorization: `Bearer ${token}`,
                        'Content-Type': 'application/json',
