...
export const AllCoursesPage = () => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const response = await axios.get(`https://student-management-ssll.onrender.com/student/${userId}/additionalCourse/all`,
          {
            method: 'GET',
            headers: {
        ...
        const coursesData = await Promise.all(
          response.data.map(async (course) => {
            const facultyProfile = await axios.get(`https://student-management-ssll.onrender.com/faculty/${course.facultyID}/profile`, {
              method: 'GET',
              headers: {
                Authorization: `Bearer ${token}`,
                'Content-Type': 'application/json',
